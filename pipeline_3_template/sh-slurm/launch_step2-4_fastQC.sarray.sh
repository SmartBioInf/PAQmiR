#!/bin/bash
#
#       launch_step2-4_fastQC.sarray.sh
#
...
if [ ! -f $sample_desc ]; then
	...
	echo "# Create sarray job file $shell_scripts/step2-4_run_fasQC.sarray.sh";
	awk -F '\t' -v umi_merged=$UMI_merged -v mod_fastQC=$FastQC "BEGIN{
			i=1;
		}{
		...
		if(i==1){
			...
			cmd1=\"module load \" mod_fastQC \"; fastqc \" umi_merged \"/\" out ;
			print cmd1;
		}
		i++;
	}" $sample_desc > $shell_scripts/step2-4_run_fasQC.sarray.sh
fi
