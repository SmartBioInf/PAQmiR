...
if [ ! -f $sample_desc ]; then
	...
	awk -F '\t' -v collapsed=$collapsed -v min=$min_length "BEGIN{
			i=1;
			print \"#!/bin/bash\";
		}{
		...
		if(i==1){
			...
			# print \"\n# Work on \" \$1 ;
			cmd1=\"cat \" collapsed \"/\" \$1 \".l\" min \".collapsed.fasta >> \" collapsed \"/all_reads.l\" min \".collapsed.fasta\" ;
			print cmd1;
		}
		i++;
...
sbatch \
-J multiqc \
-o $collapsed/merge_all.out \
-e $collapsed/merge_all.err \
--mem=2G \
--mail-type=FAIL \
