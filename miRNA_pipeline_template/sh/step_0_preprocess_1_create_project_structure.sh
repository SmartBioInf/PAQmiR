
################################
# create index folder (if needed)
echo "##### create index folder"
mkdir ../index/

# make genome download (if needed)
echo "make genome download"
./download_bosTau_8.sh

# index reference genome (if needed)
echo "index reference genome"
./index_reference_genome.sh


################################
# create mirbase folder (if needed)
echo "##### create mirbase folder"
mkdir ../mirbase21

# download mirbase (if needed)
echo "download mirbase reference database"
./step_0_preprocess_2_download_miRBase21_cow.sh



################################
# create sources folder 
echo "##### create sources folder"
mkdir ../sources


################################
# create mapper folder 
echo "##### create mapper folder"
mkdir ../mapper


################################
# create mirdeep2 folder 
echo "##### create mirdeep2 folder"
mkdir ../mirdeep2

################################
# create quantifier folder 
echo "##### create quantifier folder"
mkdir ../quantifier

################################
# create quantification_annotation folder 
echo "##### create quantification_annotation folder"
mkdir ../quantification_annotation



